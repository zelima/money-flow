...
import os
from sqlalchemy import create_engine, Column, Integer, String, Text, DECIMAL, ForeignKey, UniqueConstraint, text
...
DATABASE_URL = os.getenv("DATABASE_URL", "postgresql://budget_user:budget_pass@localhost:5432/georgian_budget")
...
class SubDepartment(Base):
    ...
    budget_allocations = relationship("SubDepartmentBudget", back_populates="sub_department", cascade="all, delete-orphan")
...
class SubDepartmentBudget(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    sub_department_id = Column(Integer, ForeignKey("sub_departments.id", ondelete="CASCADE"), nullable=False, index=True)
    ...
    __table_args__ = (UniqueConstraint('sub_department_id', 'year', name='unique_sub_dept_year'),)
...
def get_sub_departments_by_department(db, department_id: int, year: int = None):
    """Get sub-departments for a department, optionally with budget data for a specific year"""
    query = db.query(SubDepartment).filter(SubDepartment.department_id == department_id)
    
    if year:
        # Join with budget data for the specific year
        query = query.join(SubDepartmentBudget, 
...
def get_budget_drill_down(db, department_name: str = None, year: int = None, limit: int = 100):
    """
    ...
    .join(SubDepartmentBudget, SubDepartment.id == SubDepartmentBudget.sub_department_id, isouter=True)
    ...
    # Order and limit
    query = query.order_by(Department.name_english, SubDepartment.name_english, SubDepartmentBudget.year)
