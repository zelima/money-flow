# Staging Environment Configuration for Georgian Budget Application
# Reusing all modular infrastructure components
...
variable "project_id" {
  ...
  type        = string
  default     = "money-flow-469011"  # Same project as prod for now
}
...
variable "domain_name" {
  ...
  type        = string
  default     = "staging-moneyflow.thelim.dev"
}
...
variable "dns_provider" {
  ...
  type        = string
  default     = "manual"  # Set to "google" if using Cloud DNS
}

variable "notification_email" {
  description = "Email for monitoring notifications"
  type        = string
  default     = "alerts@thelim.dev"
}
...
module "storage" {
  ...
  function_source_bucket_name = "${var.project_id}-staging-function-source-${random_id.bucket_suffix.hex}"
...
module "ci_cd" {
  ...
  environment = var.environment
  domain_name = var.domain_name
  staging_domain_name = var.domain_name
  load_balancer_ip = module.networking.load_balancer_ip
  data_bucket_name = module.storage.data_bucket_name
...
locals {
  bucket_name = "${var.project_id}-staging-georgian-budget-data-${random_id.bucket_suffix.hex}"
}
